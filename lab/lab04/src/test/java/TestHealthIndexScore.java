...
public class TestHealthIndexScore {
    ...
    void testNormalBoundaryValue(String tc, int vo2Max, int rhr, int hrr) {
        System.out.println(
            tc + " " + new HealthIndexScore(vo2Max, rhr, hrr).getFitnessLevel()
        );
    ...
    @Order(2)
    @ParameterizedTest
    @CsvFileSource(files = "src/test/resources/Robustness.csv")
    @DisplayName("Robustness Testing")
    void testRobustness(String tc, int vo2Max, int rhr, int hrr) {
        System.out.println(
            tc + " " + new HealthIndexScore(vo2Max, rhr, hrr).getFitnessLevel()
        );
    }
